...
def encode_text_into_images(message, images_dir, output_dir):
    ...
    length = len(message)
    num_images = length // 4 + 1 if length % 4 > 0 else length // 4
    ...
    for i in range(0, length, 4):
        ...
        for j, image_path in enumerate(selected_images):
            ...
            # Fill the list with 301 to achieve a length of 4
            ascii_values.extend([301] * max(0, 4 - len(ascii_values)))

            # Specify the Exif data fields to use
            exif_fields = [
                '-Rating={}'.format(ascii_values[0]),
                '-YResolution={}'.format(ascii_values[1]),
                '-XResolution={}'.format(ascii_values[2]),
                '-ExifVersion={}'.format(ascii_values[3])
            ]

            # Create a copy of the original image to work with
            temp_image_file = os.path.join(output_dir, f"{i // 4 + 1}.jpg")
